...
// Matricula: 628826
// TP02 - EX11 - Quicksort PARCIAL
public class Quicksort implements Cloneable
{
	// atributos
	public static int ultimo;
	public static Quicksort [] inst;
	public static Quicksort [] retornada;
	...
	{
		Quicksort Instituicao = new Quicksort ( );
	...
	public static void ordenarQuicksort ( int esq, int dir, int k)
	{
		// declarar variaveis
		int i = esq, j = dir;	
	       	Quicksort pivo = new Quicksort ( );
		...
		while ( i <= j )
		{
			...
			while ( inst[i].nome.compareTo ( pivo.nome ) < 0 )
			{
				i++;
				comparacoes += 1;
			}
			...
			while ( inst[j].nome.compareTo ( pivo.nome ) > 0 )
			{
				comparacoes += 1;
				j--;
			}
		...
		// chamar os dois metodos novamente para ordenar
		// nao chamar o ordenar da direita caso os k desejados ja estejam em esq
		if ( esq < k )
		{
		if ( j > esq )
		{
			ordenarQuicksort ( esq, j, k );
		}
		if ( i < dir )
		{
			ordenarQuicksort ( i, dir, k );
		}
		}
		else
		{
