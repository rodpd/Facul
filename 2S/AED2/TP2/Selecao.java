...
// Matricula: 628826
// TP02 - EX08 - Ordenacao PARCIAL por Selecao
public class Selecao implements Cloneable
{
	// atributos
	public static int ultimo;
	public static Selecao [] inst;
	public static Selecao [] retornada;
	...
	{
		Selecao Instituicao = new Selecao ( );
	...
	// metodo para ordenar por selecao
	public static void ordenarSelecao ( int k )
	{
		int menor;
		Selecao instituicao = new Selecao ( );
		...
		{
			menor = i;
			// comparar elemento atual com os que ainda nao foram ordenados
			for ( int j = i+1; j <= ultimo; j++ )
			{
				if ( inst[j].codigo < inst[menor].codigo )
				{
					menor = j;
				}
				comparacoes += 1;
			}
			instituicao = inst[i].clone( );
			inst[i] = inst[menor].clone( );
			inst[menor] = instituicao.clone( );
			movimentacoes += 3;
		}
	}
