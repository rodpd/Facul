...
public class Heapsort implements Cloneable
{
	...
	// metodo para ordenar por heapsort
	public static void ordenarHeapsort ( int k )
	{
		Heapsort temp = new Heapsort( );
		// construir primeiro heap em ordem crescente
		for ( int i = 2; i <= ultimo; i++ )
		{
			for ( int j = i; j > 1 && inst[j].codigoMunicipio <= inst[j/2].codigoMunicipio; j /= 2 )
			{
				comparacoes += 1;
				// caso filho seja menor, trocar
				...
				else 
				{
					...
					{
						// trocar caso sigla do filho seja menor
						temp = inst[j].clone( );
						inst[j] = inst[j/2].clone( );
						inst[j/2] = temp.clone( );
						movimentacoes += 3;
					...
					{
						// parar loop se sigla do filho for maior
						j = 1;
					}
					comparacoes += 1;
				}
		...
		int menor = 0;
		int tam = ultimo;
		// ordenar colocando as k primeiras instituicoes nas ultimas k posicoes em ordem decrescente
		for ( int a = 0; a < k; a++ )
		{
			// trocar raiz com posicao do ultimo
			temp = inst[ultimo-a].clone( );
			inst[ultimo-a] = inst[1].clone( );
			inst[1] = temp.clone( );
			movimentacoes += 3;
			tam--;
			for ( int i = 1; i <= tam/2; i = menor )
			{
				...
				// comparar com menor filho
				if ( inst[i].getCodigoMunicipio( ) < inst[menor].getCodigoMunicipio( ) )
				{
					// parar loop se for menor
					i = tam;
				}
				else
				{
					...
					{
						// trocar se for maior
						temp = inst[menor].clone( );
						inst[menor] = inst[i].clone( );
						inst[i] = temp.clone( );
						movimentacoes += 3;
						i = menor;
					}
	...
	public static void main ( String [] args )
	{
		...
		int k = 10;
		ordenarHeapsort( k );
		// imprimir dados do array comecando da ultima posicao ate ultima-k
		for ( int i = 0; i < k; i++ )
		{
			inst[ultimo-i].imprimir( );
		}
