...
// Matricula: 628826
// TP02 - EX10 - Ordenacao por Insercao
public class Insercao implements Cloneable
{
	// atributos
	public static int ultimo;
	public static Insercao [] inst;
	public static Insercao [] retornada;
	...
	{
		Insercao Instituicao = new Insercao ( );
	...
	public static void ordenarInsercao ( int k )
	{
		Insercao temp = new Insercao ( );
		// inicializar o array ate a posicao k
		for ( int i = 1; i < k; i++ )
		{
		...
		// insercao igual a parte acima, porem com os numeros a partir de k
		for ( int i = k; i <= ultimo; i++ )
		{
			if ( inst[i].codigoMantenedora < inst[i-1].codigoMantenedora )
			{
				temp = inst[i].clone( );
				movimentacoes += 1;
				// comparar com os numeros de k para tras
				for ( int j = k-1; j>= 0; j-- )
				{
					// mover para a direita caso seja maior que o numero que esta sendo comparado
					if ( inst[j].codigoMantenedora > temp.codigoMantenedora )
