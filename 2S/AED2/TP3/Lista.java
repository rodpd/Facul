...
// Matricula: 628826
// TP02 - EX01 - Lista com Alocacao Flexivel
public class Lista implements Cloneable
{
   ...
   public void inserirInicio ( Lista temp )
   {
   ...
   public void inserirFim ( Lista temp )
   {
      ultimo.prox = temp;
      ultimo = ultimo.prox;
      temp = null;
   ...
   public void inserir ( Lista inst, int posicao )
   {
      Lista temp = primeiro;
   ...
   public Lista removerFim ( )
   {
         //clonar objeto que sera deletado
      Lista Instituicao = ultimo.clone( );
      Lista temp = primeiro;
      ...
      ultimo = temp;
      temp = ultimo.prox = null;
      return ( Instituicao );
   }
   // metodo p remover certa posicao
   public Lista remover ( int posicao )
   {
      Lista Instituicao = new Lista ( );
      Lista temp = primeiro;
      if ( primeiro != ultimo )
   ...
   {
      Lista inst = new Lista ( );
      Lista retornada = new Lista ( );
      inst.primeiro = inst;
      inst.ultimo = inst;
      ...
      for ( int i = MyIO.readInt( ); i > 0; i-- )
      {
         ...
         //instanciar objeto antes de executar metodo
            temp.ler( Integer.parseInt(s[1]) );
            inst.inserirInicio ( temp );
         }
         ...
         {
            temp.ler( Integer.parseInt(s[2]) );
            inst.inserir ( temp, Integer.parseInt(s[1]));
         }  
         ...
         // objetos retornados sao imprimidos ao serem retornados
            ...
            retornada.ultimo.prox = new Lista ( );
         ...
         {
            retornada.ultimo.prox = inst.remover ( Integer.parseInt(s[1]) );
            retornada.ultimo = retornada.ultimo.prox;
            retornada.ultimo.prox = new Lista ( );
            MyIO.println ( "(R) " + retornada.ultimo.getNome( ) );
         }
      ...
      // print do array
      for ( Lista i = inst.primeiro.prox; i != null; i = i.prox )
      {
         i.imprimir( );
      }
   }
