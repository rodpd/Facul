...
// Matricula: 628826
// TP01P02 - EX06 - Pesquisa Sequencial
public class Sequencial implements Cloneable
{
	// atributos
	public static int ultimo;
	public static Sequencial [] inst;
	public static Sequencial [] retornada;
	...
	{
		Sequencial Instituicao = new Sequencial ( );
	...
	public static void inserir ( Sequencial Instituicao )
	{
		ultimo += 1;
		inst[ultimo] = Instituicao.clone( );
	}
	// metodo para fazer pesquisa sequencial
	public static boolean pesquisaSequencial ( String linha )
	{
		boolean resp = false;
		// percorrer com todos elementos
		for ( int i = 0; i <= ultimo; i++ )
			{
				// mudar resp e parar loop caso linha seja encontrada
				if ( linha.equals ( inst[i].getSigla( ) ) )
				{
					resp = true;
					i = ultimo + 1;
				}
				// comparacoes i com ultimo e de linha com sigla do objeto
				comparacoes += 2;
			}
		// ultima comparacao com i do for
		comparacoes += 1;
		return ( resp );
	}
	public static void main ( String [] args )
	{
		...
		while ( num > 0 )
		{
			inst[ultimo+1] = new Sequencial ( );
			inst[ultimo+1].ler(num);
			ultimo += 1;
			num = MyIO.readInt( );
		}
		String linha = MyIO.readString( );
		boolean resp;
		// continuar realizando pesquisa sequencial enquanto nao encontrar FIM
		while ( ! (linha.equals ( "FIM" )) )
		{
			// resetar resposta
			resp = false;
			// procurar frase com pesquisa sequencial
			resp = pesquisaSequencial ( linha );
			if  ( resp == false )
			{
				MyIO.println ( "NÃO" );
			}
