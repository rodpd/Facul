...
public class Bubblesort implements Cloneable
{
	...
	public static Bubblesort [] inst;
	public static Bubblesort [] retornada;
	...
	{
		Bubblesort Instituicao = new Bubblesort ( );
	...
	public static void bubblesort ( )
	{
		...
		for ( int i = 0; i < ultimo; i++ )
		{
			...
			{
				if ( inst[j].getLivro( ) < inst[j+1].getLivro( ) )
				{
					temp = inst[j].clone( );
					inst[j] = inst[j+1].clone( );
					inst[j+1] = temp.clone( );
				}
				else if ( inst[j].getLivro( ) == inst[j+1].getLivro( ) )
				{
					if ( inst[j].sigla.compareTo( inst[j+1].getSigla( ) ) < 0 )
					{
						temp = inst[j].clone( );
						inst[j] = inst[j+1].clone( );
						inst[j+1] = temp.clone( );
	...
	public static void main ( String [] args )
	{
		...
		{
			inst[i].imprimir( );
			// MyIO.println ( inst[i].getLivro ( ) + " " + inst[i].getSigla( ) );
		}
