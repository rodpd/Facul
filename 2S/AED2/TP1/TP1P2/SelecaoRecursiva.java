...
// Matricula: 628826
// TP01P02 - EX09 - Ordenacao por Selecao SelecaoRecursiva SelecaoRecursiva...
public class SelecaoRecursiva implements Cloneable
{
	// atributos
	public static int ultimo;
	public static SelecaoRecursiva [] inst;
	public static SelecaoRecursiva [] retornada;
	...
	public SelecaoRecursiva clone ( )
	{
		SelecaoRecursiva Instituicao = new SelecaoRecursiva ( );
	...
	// metodo para ordenar por selecao recursiva
	public static void selecaoRecursiva ( int i, int j, int menor )
	{
		// continuar enquanto i for menor que ultimo para percorrer com todos elementos
		if ( i <= ultimo )
		{
			// comparar com os outros e encontrar menor
			if ( j <= ultimo )
			{
				if ( inst[j].getCodigo( ) < inst[menor].getCodigo( ) )
				{
					menor = j;
				}
				// chamar metodo novamente para comparar com os proximos
				comparacoes += 1;
				selecaoRecursiva ( i, j+1, menor );
			}
			// chamar metodo novamente apenas caso j seja igual ultimo
			if ( j == ultimo )
			{
			// trocar posicao i pelo menor
			SelecaoRecursiva temp = new SelecaoRecursiva ( );
			temp = inst[i].clone( );
			inst[i] = inst[menor].clone( );
			inst[menor] = temp.clone( );
			movimentacoes += 3;
			// aumentar i e igualar a menor para recomecar em uma posicao maior
			i++;
			menor = i;
			selecaoRecursiva ( i, i+1, menor );
			}
		}
	}
	public static void main ( String [] args )
	{
		...
		{
			comparacoes += 1;
			inst[ultimo+1] = new SelecaoRecursiva ( );
			inst[ultimo+1].ler(num);
			ultimo += 1;
			num = MyIO.readInt( );
		}
		// ordenar com selecao recursiva
