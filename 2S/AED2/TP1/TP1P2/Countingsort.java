...
// Matricula: 628826
// TP01P02 - EX14 - Counting Sort
public class Countingsort implements Cloneable
{
	// atributos
	public static int ultimo;
	public static Countingsort [] inst;
	public static Countingsort [] retornada;
	...
	{
		Countingsort Instituicao = new Countingsort ( );
	...
	{
		// descobrir maior numero para ser tamanho do array de contagem
		int maior = 0;
		for ( int i = 0; i <= ultimo; i++ )
		{
			if ( inst[i].getPeriodico( ) > maior )
			{
				maior = inst[i].getPeriodico( );
			}
		}
		int tam = maior+1;
		int [] contagem = new int [tam];
		Countingsort [] saida = new Countingsort [ultimo+1];
		// zerar array de contagem
		for ( int i = 0; i < 45001; i++ )
		{
			contagem[i] = 0;
		}
		// inicializar array de saida
		for ( int i = 0; i <= ultimo; i++ )
		{
			saida[i] = new Countingsort ( );
		}
		// contar quantidades
		for ( int i = 0; i <= ultimo; i++ )
		{
			contagem[inst[i].getPeriodico( )]++;
		}
		// fazer array de contagem ser cumulativo
		for ( int i = 1; i < tam; i++ )
		{
			contagem[i] += contagem[i-1];
		}
		// passar do array de entrada para o de saida
		for ( int i = ultimo; i >= 0; i-- )
		{
			saida[contagem[inst[i].getPeriodico( )]-1] = inst[i].clone( );
			contagem[inst[i].getPeriodico( )]--;
		}
		/* int cont;
		for ( int i = 0; i < ultimo; i++ )
		...
		contagem = null;
		{
			while ( saida[i].getPeriodico( ) == saida[i+1].getPeriodico( ) )
			{
				cont++;
				i++;
			}
			contagem = null;
			contagem = new int [cont];
		} */
		// igualar array original ao de saida
		for ( int i = 0; i <= ultimo; i++ )
		{
			inst[i] = saida[i].clone( );
			movimentacoes++;
		}
	}
