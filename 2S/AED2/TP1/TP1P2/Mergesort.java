...
// Matricula: 628826
// TP01P02 - EX16 - Mergesort
public class Mergesort implements Cloneable
{
	// atributos
	public static int ultimo;
	public static Mergesort [] inst;
	public static Mergesort [] retornada;
	...
	{
		Mergesort Instituicao = new Mergesort ( );
	...
	public static void ordenarMergesort ( int esq, int dir)
	{
		int meio;
		if ( esq < dir )
		{
			meio = ( esq+dir ) / 2;
			ordenarMergesort ( esq, meio );
			ordenarMergesort ( meio+1, dir );
			int i, j, k;
			int tam1 = meio-esq+1;
			int tam2 = dir-meio;
			Mergesort [] temp = new Mergesort[tam1];
			Mergesort [] temp2 = new Mergesort[tam2];
			// popular arrays auxiliares com partes do array que serao comparadas
			for ( i = 0; i < tam1; i++ )
			{
				temp[i] = inst[esq+i].clone( );
            movimentacoes++;
			}
			for ( j = 0; j < tam2; j++ )
			{
				temp2[j] = inst[meio+j+1].clone( );
            movimentacoes++;
			}
			i = 0;
			j = 0;
			k = esq;
			// continuar enquanto nao tiver percorrido ao menos um dos arrays
			while ( i < tam1 && j < tam2 )
			{
				// comparar arrays e mudar o original
				if ( temp[i].getReceita( ) <= temp2[j].getReceita( ) )
				{
					inst[k] = temp[i].clone( );
					i++;
				}
			...
			// preencher original com o que restou do outro
			while ( i < tam1 )
			{
				inst[k] = temp[i].clone( );
				i++;
	...
	public static void main ( String [] args )
	{
		...
		while ( num > 0 )
		{
			inst[ultimo+1] = new Mergesort ( );
			inst[ultimo+1].ler(num);
			ultimo += 1;
			num = MyIO.readInt( );
		}
		// ordenar por quicksort
		ordenarMergesort( 0, ultimo );
