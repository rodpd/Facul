...
Tempo Sequencial -> 0m22.965s

Tempo Paralelo 1 Thread -> 0m23.708s

Tempo Paralelo 2 Threads -> 0m28.022s

Tempo Paralelo 4 Threads -> 0m26.167s

Tempo Paralelo 8 Threads -> 1m7.798s
...
{
#pragma omp parallel for simd num_threads(numThreads) schedule(dynamic,30)
  for(int i =0;i < Numero_Neuronios;i++)
...
void Camada :: Ajustar_Pesos_Neuronios(double Erros[], double Entrada[])
{
  ...
  /* C�lculo do Somat�rio que ser� usado para o c�lculo do erro
     da camada anterior */
//#pragma omp parallel for collapse(2) num_threads(numThreads) schedule(dynamic,1000)
  for(int i=1;i < Numero_Pesos;i++)
  {
    Temp  = 0;
    for(int j=0;j < Numero_Neuronios;j++)
    {
...
//#pragma omp parallel for num_threads(numThreads) schedule(dynamic,50)
  ...
  /* Atribui o vetor de erros calculado, para o vetor erro
     que ser� retornado */
//#pragma omp parallel for num_threads(numThreads)
  for(int i=0;i < (Numero_Pesos -1);i++)
    Erros[i] = Erro_Aux[i];
}
...
void Camada :: Retornar_Saida(double Linha[])
{
  Linha[0] = 1;
  //#pragma omp parallel for num_threads(numThreads)
  for(int i =1;i <= Numero_Neuronios;i++)
    Linha[i] = Saida[i-1];
}
             ...
             int Numero_Colunas_Entrada, int Numero_Colunas_Saida, int Numero_Neuronio_Camada[])
{
  ...
  for(int i = 0; i < Numero_Linhas; i++)
    for(int j = 0; j < Numero_Colunas_Entrada; j++)
      fread(&X[i][j],sizeof(double), 1, Entrada);

  for(int i = 0; i < Numero_Linhas; i++)
    for(int j = 0; j < Numero_Colunas_Saida; j++)
...
//#pragma omp parallel for num_threads(numThreads)
  for(int i = 1; i < Numero_Camadas; i++)
...
int main(void)
{
  ...
  while(Continua != 'n')
  {
    ...
    if(Continua == 'r')
    {
     ...
     Numero_Camadas = 3;
     Numero_Neuronio_Camada[0] = 75;
     Numero_Neuronio_Camada[1] = 75;
     Numero_Neuronio_Camada[2] = 75;
